...
export default function Closure({ user }: ClosureProps) {
  ...
  // Filter completed initiatives only
  const completedInitiatives = initiatives.filter((i: any) => 
    i.status === 'Completed' || i.status === 'Implemented' || i.status === 'Validated'
  );
  ...
  const handleSelectInitiative = (initiative: any) => {
    ...
    setClosureData({
      ...closureData,
      initiativeNumber: initiative.id.toString(),
          ...
          <Card>
            <CardHeader>
              ...
              <p className="text-sm text-muted-foreground">
                Only completed initiatives are available for closure
              </p>
            </CardHeader>
            <CardContent>
              {isLoading ? (
                ...
                <div className="text-center py-8 text-muted-foreground">
                  No completed initiatives available for closure
                </div>
              ) : (
              ...
              <CardHeader>
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      Complete the structured closure process for {selectedInitiative.title}
                    </p>
                  </div>
